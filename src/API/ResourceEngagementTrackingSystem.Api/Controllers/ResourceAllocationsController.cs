...
namespace ResourceEngagementTrackingSystem.Api.Controllers
{
    ...
    public class ResourceAllocationsController : ControllerBase
    {
        ...
        public ResourceAllocationsController(IResourceAllocationService service)
        {
            _service = service;
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] UpdateResourceAllocationDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
